...
function App() {
  ...
  const [conciergeMessage, setConciergeMessage] = useState('I have 3 hours and want something historic and local.')
      ...
      const response = await fetch(import.meta.env.VITE_AI_API_URL || 'http://localhost:8000/chat', {
        ...
        body: JSON.stringify({
          ...
          context: {
            ...
            interests: [],
            transport_mode: activeTransport === 'Local transit' ? 'transit' : 'walking',
          },
        }),
      })
  ...
  return (
    <main className="app-shell">
      ...
      <section className="concierge-section" id="concierge"><div className="section-heading"><div><span className="section-number">04</span><h2>Ask the concierge</h2></div><p>Describe the kind of afternoon you want. We will shape it around your time and budget.</p></div><form className="concierge-form" onSubmit={askConcierge}><label htmlFor="concierge-message">What are you in the mood for?</label><textarea id="concierge-message" value={conciergeMessage} maxLength={2000} onChange={(event) => setConciergeMessage(event.target.value)} /><button className="primary-button" type="submit" disabled={isPlanning || !conciergeMessage.trim()}>{isPlanning ? 'Building your adventure…' : 'Build my adventure'} <ArrowRight size={17} /></button></form>{conciergeError && <p className="concierge-error" role="alert">{conciergeError}</p>}{conciergeResponse && <article className="concierge-result"><div className="concierge-result-heading"><div><span className="eyebrow">Your field notes</span><h3>{conciergeResponse.plan.title}</h3></div><span className="concierge-cost">{destinationPlace.symbol}{conciergeResponse.plan.estimated_cost.toLocaleString()} · {conciergeResponse.plan.duration_minutes} min</span></div><p>{conciergeResponse.message}</p><div className="concierge-stops">{conciergeResponse.plan.stops.map((stop) => <div className="concierge-stop" key={stop.name}><strong>{stop.name}</strong><span>{stop.duration_minutes} min · {destinationPlace.symbol}{stop.estimated_cost.toLocaleString()}</span><small>{stop.description}</small></div>)}</div></article>}</section>
