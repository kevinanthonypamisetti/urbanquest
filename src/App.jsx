...
function App() {
  ...
  if (path === '/login' || path === '/signup') return <AuthPage mode={path.slice(1)} />
  return <PlannerApp />
}
...
function CookiePreferences({ onSave }) {
  ...
  const [open, setOpen] = useState(false)
  return <aside className="cookie-banner" aria-label="Cookie preferences"><div><strong>Cookies & privacy</strong><p>UrbanQuest uses essential cookies to keep you signed in and remember preferences.</p></div>{open && <label className="cookie-toggle"><span>Analytics</span><input type="checkbox" checked={analytics} onChange={(event) => setAnalytics(event.target.checked)} /></label>}<div className="cookie-actions"><button onClick={() => onSave('essential')}>Essential only</button>{open ? <button className="cookie-primary" onClick={() => onSave(analytics ? 'all' : 'essential')}>Save choices</button> : <button className="cookie-primary" onClick={() => onSave('all')}>Accept all</button>}<button onClick={() => setOpen(!open)}>{open ? 'Close' : 'Manage'}</button></div></aside>
}
